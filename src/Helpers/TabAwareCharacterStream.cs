...
namespace HtmlTools
{
    ...
    public class TabAwareCharacterStream
    {
        ...
        public TabAwareCharacterStream(string text, int tabWidth = 4)
            : this(new StringTextProvider(text), tabWidth) { }
        ...
        public char PrevChar => Position == 0 ? '\0' : Text[Position - 1];
        public char NextChar => Position >= Length - 1 ? '\0' : Text[Position + 1];
        ...
        private class TabAwarePeeker : StreamPeeker
        {
            private readonly int remainingSpaces;
            public TabAwarePeeker(TabAwareCharacterStream stream)
                : base(stream)
            {
                remainingSpaces = stream.remainingSpaces;
            }
            protected override void Revert()
            {
                Stream.remainingSpaces = remainingSpaces;
            }
        }
        ...
        public bool TryConsumeWhiteSpace(int width)
        {
            if (remainingSpaces > 0)
            {
                ...
                {
                    using (StreamPeeker peek = Peek())
                    {
                        int consumed = remainingSpaces;
